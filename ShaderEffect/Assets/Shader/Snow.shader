Shader "YYCustom/Snow"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float value = dot(i.wNormal,_SnowDir);
                value = step(_ControlValue.x,value);
                float2 st = i.uv * _ControlValue.y;
                float noiseValue = noise(st);
                noiseValue = step(noiseValue,_ControlValue.z);
                value = value * noiseValue;
                return lerp(_Color,_SnowColor,value);
            }
            ENDHLSL
        }
    }
